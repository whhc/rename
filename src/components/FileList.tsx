...
export const FileList: React.FC<FileListProps> = ({
  ...
  const [selectedFiles, setSelectedFiles] = useState<Set<FormatedItem>>(
    new Set(),
  );
  ...
  const handleOpen = (file: FormatedItem) => {
    let path = file.path;
    // 仅限windows系统
    path = path.replace(/\\[^\\]*?$/, "");
        ...
        <div className="col-span-1">
          <input
            ...
            checked={
              ...
              selectedFiles.size === formatedList.length
            }
          ...
          formatedList.map((file, index) => (
            <div key={index} className="grid grid-cols-12 gap-4 p-2 border-b">
              ...
              <div className="col-span-4 truncate text-sm">
                <TooltipWrap tip={file.file || ""}>{file.file}</TooltipWrap>
              </div>
              <div className="col-span-6 truncate text-sm">
                {rule ? (
                  <TooltipWrap tip={file.renamed || ""}>
                    {file.renamed}
                  </TooltipWrap>
                ) : null}
              </div>
